...
{
    connect(Uart, &QSerialPort::readyRead, this, &Uart_RecvData::UartRecvData, Qt::QueuedConnection);
}

void Uart_RecvData::UartRecvData()
{
    QByteArray recvData= Uart->readAll();
    /* 当接收数据不为空，将接收数据处理后发送出去 */
    if ((!recvData.isEmpty()))
    {
        bool     ok=true;
        ...
        QString  RecvDatastr = QString(recvData.toHex().trimmed().toUpper());
